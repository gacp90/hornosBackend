...
const getTemperaturasInterval = async (req, res = response) => {
    try {
        ...
        if (!registros.length) {
            return res.json({ ok: true, temperaturas: [], total: 0 });
        }

        // Agrupar los registros por intervalos de tiempo
        ...
        while (fechaActual.isBefore(fechaFin)) {
            ...
            // Filtrar registros dentro de este intervalo
            const registrosEnLapso = registros.filter(
                (r) => moment(r.fecha).isBetween(fechaActual, fechaLapsoFin, null, "[)")
            );

            // Tomar el último registro del intervalo
            if (registrosEnLapso.length) {
                temperaturas.push(registrosEnLapso[registrosEnLapso.length - 1]);
            }
